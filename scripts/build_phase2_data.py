...
assets=[]
assets.append(write('fig-03-experiment-scale.json',{
 'source_population':50000,
 'counts':{'whole_system':len(whole),'competent_rock_family_pool':len(fam),'pareto_eligible':2585,'pareto_nondominated':len(pareto),'hydraulic_jump':len(jump),'alluvial':len(alluv),'alluvial_common_overlap':int(alluv['comparison_overlap'].sum())},
 'note':'Counts are study-population context; samples shown in later charts are display reductions.'}))
assets.append(write('fig-04-head-response.json',{
 'equation':'H=(Q/(C L))^(2/3)','elasticities':{'Q':2/3,'L':-2/3,'C':-2/3},
 'ranges':{'Q_m3s':[float(whole.Q_m3s.min()),float(whole.Q_m3s.max())],'overflow_length_m':[float(whole.overflow_length_m.min()),float(whole.overflow_length_m.max())],'C_bcw':[float(whole.C_bcw.min()),float(whole.C_bcw.max())]},
 'sobol':recs(sob[sob.response.eq('head_m')].sort_values('ST',ascending=False).head(5),['variable','S1','ST','S1_lo95','S1_hi95','ST_lo95','ST_hi95'])}))
assets.append(write('fig-05-forcing-response.json',{
 'relationship':{'form':"P' proportional to q^1.67 Fr1^1.87",'q_exponent':1.67,'Fr1_exponent':1.87,'r2':0.9999,'pct_effect_10pct_q':17.0,'pct_effect_10pct_Fr1':19.5},
 'ranges':{'q_m2s':[float(whole.unit_discharge_m2s.min()),float(whole.unit_discharge_m2s.max())],'Fr1':[float(whole.Fr1.dropna().min()),float(whole.Fr1.dropna().max())]},
 'sobol':recs(sob[sob.response.eq('forcing_kW_m')].sort_values('ST',ascending=False).head(6),['variable','S1','ST','interaction_gap_ST_minus_S1'])}))
assets.append(write('fig-06-body-area.json',{
 'equation':'A=P*T+0.5*s*P^2','normalized':'A/P^2=T/P+0.5*s','exact_identity':True,
 'sobol':recs(sob[sob.response.eq('body_area_m2_per_m')].sort_values('ST',ascending=False).head(6),['variable','S1','ST'])}))
...
assets.append(write('fig-08-pareto-trade-space.json',{'eligible_population':2585,'nondominated_n':len(pareto),'objectives':['body_area_m2_per_m','forcing_kW_m','tailwater_mismatch_abs','rock_threshold_ratio'],'points':recs(pareto,['scenario_id','body_area_m2_per_m','forcing_kW_m','tailwater_mismatch_abs','rock_threshold_ratio','retention_fraction','structure_height_m','base_width_m','top_width_m','Q_m3s','overflow_length_m','Fr1']) ,'caveat':'No scalar winner; exact nondominated membership is sample-sensitive.'}))
assets.append(write('fig-09-family-map.json',{'eligible_population':10317,'display_population':min(800,len(fam)),'points':recs(fam.sort_values('scenario_id'),['scenario_id','family_neighborhood','B_over_P','T_over_P','head_over_P','Fr1'],800),'envelopes':recs(env,list(env.columns)),'family_shares_pct':{r['family']:round(float(r['share_pct']),3) for _,r in env.iterrows()}}))
...
assets.append(write('fig-12-hydraulic-jump.json',{'population':len(jump),'curve':recs(jump,['Fr1','y2_y1','dE_y1','Lcwc_y1','Lusace_y1','Lcwc_Lusace'],320),'quantiles':recs(jq,list(jq.columns)),'domain':{'Fr1':[4.5,9.0]},'caveat':'Reference scaling only; not final IS 4997:2026 basin dimensions.'}))
...
for resp in keepresp:
    z=sob[sob.response.eq(resp)].sort_values('ST',ascending=False).head(6)
...
schema={'runtime_policy':'Compact JSON for browser rendering. No source CSV is copied; original raster figures are not read.',
        'figures':{a['path'].split('/')[-1].replace('.json',''):{'path':a['path'],'format':'json'} for a in assets}}
...
public_csv_policy={'source_csv_copied':False,'raster_used_as_data':False,'runtime_format':'json'}
audit={'status':'PASS','checks':checks,'policy':public_csv_policy,'notes':['FIG-01 and FIG-02 use existing metadata JSON; FIG-03 through FIG-17 have compact runtime JSON.','Full authoritative CSVs remain build inputs and are not copied into the public runtime layer.']}
