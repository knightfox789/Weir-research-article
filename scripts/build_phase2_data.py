...
assets.append(write('fig-08-pareto-trade-space.json',{'eligible_population':2585,'nondominated_n':len(pareto),'objectives':['body_area_m2_per_m','forcing_kW_m','tailwater_mismatch_abs','rock_threshold_ratio'],'points':recs(pareto,['scenario_id','body_area_m2_per_m','forcing_kW_m','tailwater_mismatch_abs','rock_threshold_ratio','retention_fraction']) ,'caveat':'No scalar winner; exact nondominated membership is sample-sensitive.'}))
assets.append(write('fig-09-family-map.json',{'eligible_population':10317,'display_population':min(120,len(fam)),'points':recs(fam.sort_values('scenario_id'),['scenario_id','family_neighborhood','B_over_P','T_over_P'],120),'envelopes':recs(env,list(env.columns)),'family_shares_pct':{r['family']:round(float(r['share_pct']),3) for _,r in env.iterrows()}}))
...
assets.append(write('fig-12-hydraulic-jump.json',{'population':len(jump),'curve':recs(jump,['Fr1','y2_y1','dE_y1','Lcwc_y1','Lusace_y1','Lcwc_Lusace'],100),'quantiles':recs(jq,list(jq.columns)),'domain':{'Fr1':[4.5,9.0]},'caveat':'Reference scaling only; not final IS 4997:2026 basin dimensions.'}))
